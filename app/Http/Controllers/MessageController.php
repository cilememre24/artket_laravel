...
class MessageController extends Controller
{
    public function index(){
        ...
        from users LEFT  JOIN  messages ON users.id = messages.sender_id and is_read = 0 and messages.receiver_id = " . $current_user_id . "
        where users.id != " . $current_user_id . " 
        group by users.id, users.username, users.imgfile_path, users.first_name,users.last_name");

        return view('chat',['all_people' => $all_people,'current_user_id' => $current_user_id]);
    }
